...
exports.post=function(req,res,next){
    ...
    function(callback){
    	var query='select songId,name,src,count(songLikesId) as songLikes from mediadb.song '+
				...
				'where song.name like "'+req.body.searchStr+'%" group by song.name order by addDate DESC';
    ...
    function(songsIds,songs,callback){
		...
		connection.query(query,function(err,data){
			...
			callback(err,songs,songsIds);
		})
    },
    function(songs,songsIds,callback){
		...
		connection.query(query,function(err,data){
			...
			songs=songs.map(function(item){
				item.canFavorite = (data.indexOf(item.songId)===-1) ? true : false;
				return item;
			})
			callback(err,songs);
		})
